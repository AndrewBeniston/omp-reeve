...
export type QueuedMessages = QueuedMessageSnapshot;

function normalizeQueuedMessages(
  q?: QueuedMessageSnapshot | { steering?: string[]; followUp?: string[] } | null,
): QueuedMessages {
  if (q && "items" in q) return { items: q.items ?? [], paused: Boolean(q.paused) };
  const steering = q?.steering ?? [];
  const followUp = q?.followUp ?? [];
  const legacyItem = (kind: "steer" | "followUp", text: string, index: number): QueuedMessageItem => ({
  ...
  return {
    items: [
      ...steering.map((text, index) => legacyItem("steer", text, index)),
      ...followUp.map((text, index) => legacyItem("followUp", text, index)),
    ],
    paused: false,
  };
}
...
type ModelEntry = { id: string; name: string; provider: string; fastModeFamily?: FastModeFamily | null };
type ModelsResponse = {
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const fastModeFamily = displayModel
    ? modelList.find((entry) => entry.provider === displayModel.provider && entry.id === displayModel.modelId)?.fastModeFamily ?? null
    : null;
  const fastModeAvailable = fastModeFamily !== null;
  ...
  useEffect(() => {
    ...
    setFastModeEnabled(data?.context.serviceTierByFamily?.[fastModeFamily] === "priority");
  }, [data?.context.serviceTierByFamily, fastModeFamily]);
    ...
    try {
      ...
      try {
        ...
        if (liveState) {
          ...
          if (liveState.collaboration?.active) {
            collaborationActiveRef.current = true;
            setMessages((current) => applyCollaborationSnapshot(current, liveState.collaboration!, {
      ...
      es.onerror = () => {
        ...
        if (es.readyState === EventSource.CLOSED) {
          ...
          if (eventSourceRef.current === es && (
            agentRunningRef.current || eventStreamGraceActiveRef.current || collaborationActiveRef.current
          )) {
            eventSourceRef.current = null;
            eventSourceSessionIdRef.current = null;
            eventConnectionAttemptRef.current = null;
  ...
  const addNotice = useCallback((notice: { id?: string; message: string; type?: NoticeType; actionLabel?: string; onAction?: () => void }) => {
  ...
  }, [addNotice, chatInputRef, onAttentionNeeded]);
  ...
  const scheduleEventStreamClose = useCallback((sid: string) => {
    ...
    const checkServerIdle = async () => {
      ...
      try {
        ...
        collaborationActiveRef.current = Boolean(state?.collaboration?.active);
  ...
  const handleAgentEvent = useCallback((event: AgentEvent) => {
    switch (event.type) {
      ...
      case "session_name_changed":
        {
          ...
          if (sid && name) onSessionNameChanged?.(sid, name);
        }
        break;
      case "prompt_error":
      ...
      case "queue_update":
        setQueuedMessages(normalizeQueuedMessages(
          (event.queuedMessages as QueuedMessageSnapshot | undefined)
          ?? {
      ...
      case "collab_status":
        {
          ...
          if (isCollaborationSnapshot(collaboration)) {
            collaborationActiveRef.current = collaboration.active;
            setMessages((current) => applyCollaborationSnapshot(current, collaboration));
          }
        }
        break;
    }
  ...
  const handleSend = useCallback(async (message: string, images?: AttachedImage[], attachments?: ComposerAttachmentDescriptor[], sentAsGoal = false) => {
    const trimmedMessage = message.trim();
    if (!trimmedMessage && !images?.length && !attachments?.length) return false;
  ...
  const handleGoalSubmit = useCallback(async (
    ...
    const sent = await handleSend(input.objective, attachments, undefined, true);
 ...
 const handleCompact = useCallback(async () => {
      ...
      switch (commandName) {
       case "compact": {
        ...
        case "collab": {
          if (!sid) return complete({ handled: true, error: "No active session" });
          await ensureEventsConnected(sid);
          const result = await sendAgentCommand<CollaborationCommandResult>(sid, {
            type: "collab",
            args,
          });
          setMessages((current) => applyCollaborationSnapshot(current, result.collaboration, {
            appendIfMissing: true,
            message: result.message,
          }));
          collaborationActiveRef.current = result.collaboration.active;
          if (!result.collaboration.active) scheduleEventStreamClose(sid);
          return result.collaboration.active
            ? { handled: true }
  ...
  const applyQueueSnapshot = useCallback((snapshot?: QueuedMessageSnapshot | null) => {
  ...
  const handleDeleteQueuedMessage = useCallback(async (id: string) => {
    const sid = sessionIdRef.current;
    if (!sid) return null;
    try {
      const result = await sendAgentCommand<{ undoToken: string; queuedMessages: QueuedMessageSnapshot }>(sid, {
        type: "delete_queue_item",
        id,
      });
      applyQueueSnapshot(result?.queuedMessages);
      if (result?.undoToken) {
        addNotice({
          type: "info",
          message: translate("chat.queueDeleted"),
          actionLabel: translate("chat.undo"),
          onAction: () => {
            void sendAgentCommand<QueuedMessageSnapshot>(sid, {
              type: "undo_delete_queue_item",
      ...
      addNotice({ type: "error", message: error instanceof Error ? error.message : "Failed to delete queued message" });
      return null;
    }
  }, [addNotice, applyQueueSnapshot, translate]);

  const handleUndoDeletedQueuedMessage = useCallback(async (undoToken: string) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, { type: "undo_delete_queue_item", undoToken });
    applyQueueSnapshot(result);
  }, [applyQueueSnapshot]);

  const handleEditQueuedMessage = useCallback(async (id: string): Promise<QueuedMessageDraft | null> => {
    const sid = sessionIdRef.current;
    if (!sid) return null;
    try {
      const result = await sendAgentCommand<QueuedMessageDraft & { queuedMessages: QueuedMessageSnapshot }>(sid, {
        type: "begin_queue_edit",
        id,
      });
      applyQueueSnapshot(result?.queuedMessages);
      return result ?? null;
    } catch (error) {
      addNotice({ type: "error", message: error instanceof Error ? error.message : "Failed to edit queued message" });
      return null;
    }
  }, [addNotice, applyQueueSnapshot]);

  const handleCancelQueuedMessageEdit = useCallback(async (editToken: string) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, { type: "cancel_queue_edit", editToken });
    applyQueueSnapshot(result);
  }, [applyQueueSnapshot]);

  const handleCompleteQueuedMessageEdit = useCallback(async (
    editToken: string,
    message: string,
    images?: AttachedImage[],
  ) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const piImages = images?.map((image) => ({ type: "image" as const, data: image.data, mimeType: image.mimeType }));
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, {
      type: "complete_queue_edit",
      editToken,
      message,
      ...(piImages?.length ? { images: piImages } : {}),
    });
  ...
  const handleReorderQueuedMessages = useCallback(async (ids: string[]) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, { type: "reorder_queue_items", ids });
    applyQueueSnapshot(result);
  }, [applyQueueSnapshot]);

  const handleSendQueuedMessageNow = useCallback(async (id: string) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, { type: "send_queue_item_now", id });
    applyQueueSnapshot(result);
  }, [applyQueueSnapshot]);
  ...
  const handleResumeQueuedMessages = useCallback(async () => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, { type: "resume_queue" });
    applyQueueSnapshot(result);
  }, [applyQueueSnapshot]);

  const handleResolvePausedQueueSubmission = useCallback(async (clearQueue: boolean) => {
    const sid = sessionIdRef.current;
    if (!sid) return;
    const result = await sendAgentCommand<QueuedMessageSnapshot>(sid, {
      type: "resolve_paused_queue_submission",
      clearQueue,
    });
    applyQueueSnapshot(result);
  }, [applyQueueSnapshot]);
  ...
  const handleCycleThinkingLevel = useCallback(async () => {
    ...
    if (!sid) return;
    try {
      const result = await sendAgentCommand<{ supported: boolean; level?: ThinkingLevelOption }>(sid, {
        type: "cycle_thinking_level",
      });
      if (result.level) {
        setThinkingLevel(result.level);
      } else if (!result.supported) {
        addNotice({ type: "info", message: translate("chat.noEffortLevels") });
      }
    } catch (error) {
      console.error("Failed to cycle the effort level:", error);
      addNotice({ type: "error", message: translate("chat.effortCycleFailed") });
    }
  ...
  const handleApprovalModeChange = useCallback(async (mode: ApprovalMode): Promise<boolean> => {
    ...
    try {
      const response = await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ path: "tools.approvalMode", value: mode }),
      });
  ...
  const handleFastModeChange = useCallback(async (enabled: boolean) => {
    ...
    try {
      const result = await sendAgentCommand<{ supported: boolean; enabled: boolean }>(sid, {
        type: "set_fast_mode",
        enabled,
      });
      setFastModeEnabled(result.enabled);
      if (!result.supported) {
        addNotice({ type: "info", message: translate("chat.fastUnavailable") });
      }
    } catch (error) {
      setFastModeEnabled(previous);
      console.error("Failed to set fast mode:", error);
      addNotice({ type: "error", message: translate("chat.fastChangeFailed") });
    }
  }, [addNotice, ensureNewSession, fastModeEnabled, translate]);
    ...
    if (session) {
      ...
      loadSession(session.id, true, true).then((agentState) => {
        if (agentState?.running) {
          ...
          if (agentState.state?.collaboration?.active) {
            collaborationActiveRef.current = true;
            void connectEvents(session.id);
          }
        }
        ...
        if (state) {
          if (state.approvalMode !== undefined) setApprovalMode(state.approvalMode);
          ...
          if (state.fastModeEnabled !== undefined) setFastModeEnabled(state.fastModeEnabled);
          if (state.extensionStatuses !== undefined) setExtensionStatuses(state.extensionStatuses ?? []);
          if (state.extensionWidgets !== undefined) setExtensionWidgets(state.extensionWidgets ?? []);
  ...
  useEffect(() => {
    const approvalCwd = newSessionCwd ?? session?.cwd;
    if (!approvalCwd) {
    ...
    const controller = new AbortController();
    setApprovalModeError(null);
    fetch(`/api/settings?cwd=${encodeURIComponent(approvalCwd)}`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok || data.error) throw new Error(data.error ?? `HTTP ${response.status}`);
        setApprovalMode(approvalModeFromSettings(data));
      })
      .catch((error) => {
  ...
  return {
    ...
    handleToolPresetChange, handleApprovalModeChange, handleThinkingLevelChange, handleCycleThinkingLevel, handleFastModeChange, loadTools, loadSlashCommands, ensureNewSession, setActiveLeafId, setData, setMessages,
