...
test("a completed model switch reloads canonical session state and reports failures", () => {
  ...
  assert.match(switchSource, /console\.error\("Failed to set model:", e\)/);
  assert.match(switchSource, /addNotice\(\{ type: "error", message: translate\("composer\.modelSettings\.errorGeneric"\) \}\)/);
});
