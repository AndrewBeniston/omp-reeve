...
import { hasCompleteWebThemeVariables, WEB_THEME_VARIABLE_NAMES } from "@/lib/theme-contract";
...
function readCachedThemeConfig(theme: ResolvedTheme): WebThemeConfig | null {
  try {
    const cached = localStorage.getItem(THEME_CONFIG_KEY);
    if (!cached) return null;
    const config = JSON.parse(cached) as WebThemeConfig | null;
    const candidate = config?.palettes?.[theme];
    return candidate && hasCompleteWebThemeVariables(candidate.variables) ? config : null;
  } catch {
    return null;
  }
}
...
function applyOmpPalette(theme: ResolvedTheme): void {
  ...
  const candidate = themeConfig?.palettes[theme];
  const palette = candidate && hasCompleteWebThemeVariables(candidate.variables) ? candidate : null;
  ...
  if (palette) {
    ...
    delete root.dataset.ompThemeName;
  }
...
function ensureState(): ThemeState {
  ...
  themeConfig = readCachedThemeConfig(theme);
...
export function useTheme(options?: { cwd?: string | null; syncWithOmp?: boolean }) {
  ...
  useEffect(() => {
    ...
    void refreshOmpTheme(options.cwd).catch(() => {
      if (cancelled) return;
      const current = ensureState();
      themeConfig = readCachedThemeConfig(current.theme);
      applyOmpPalette(current.theme);
    });
    return () => {
      cancelled = true;
    };
