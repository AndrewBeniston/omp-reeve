...
<title>Reeve · authentication required</title>
<style>
...
<main>
  ...
  <p>Reeve is locked. Reload the page and sign in with the username <code>omp</code> and your password.</p>
  <p>Forgot it? <a href="${RECOVERY_PAGE}">Recover access</a> — you will need to read a one-time code off the console
     of the machine running Reeve.</p>
</main>
</body>
</html>
`;
...
function isDesktopHealthRequest(request: NextRequest): boolean {
  const challenge = request.headers.get(DESKTOP_CHALLENGE_HEADER);
  return request.method === "GET"
    && Boolean(process.env.OMP_WEB_DESKTOP_TOKEN)
    && typeof challenge === "string"
    && UUID_PATTERN.test(challenge);
}
...
export function proxy(request: NextRequest) {
  ...
  if (pathname === DESKTOP_HEALTH_API && isDesktopHealthRequest(request)) {
  ...
  if (decision === "unavailable") {
    const message = "Password access is enabled but the Reeve credential file could not be read."
      + " Run `reeve --reset-password` on the server to set a new password.";
    return isApiRequest
