...
import {
  createLiveControllerBridge,
  getLiveControllerBridge,
  getOrCreateLiveControllerBridge,
  shutdownLiveControllerBridges,
} from "./live-controller-bridge.ts";
...
test("destroying a Wrapper closes and removes its live controller bridge", async () => {
  const { services, controls } = liveServices();
  const inner = { sessionId: "session-6", isBashRunning: false, dispose: async () => {} };
  const bridge = getOrCreateLiveControllerBridge(inner.sessionId, inner, services);
  await bridge.start();
  const wrapper = new AgentSessionWrapper(inner, { on: () => () => {} });

  wrapper.destroy();
  assert.equal(getLiveControllerBridge(inner.sessionId), undefined);
