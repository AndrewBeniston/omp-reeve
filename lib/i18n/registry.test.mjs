...
test("ships the complete English translation key set in every locale", () => {
  const englishKeys = Object.keys(getLocalePlugin("en").messages).sort();

  for (const locale of getSupportedLocales().filter((locale) => locale !== "en")) {
    assert.deepEqual(
      Object.keys(getLocalePlugin(locale).messages).sort(),
      englishKeys,
