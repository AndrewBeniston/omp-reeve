...
test("resolves the credential path from the agent directory, and honours the override", () => {
  // resolve, not join. The store resolves the directory, and Windows adds the
  // current drive letter to a path that starts with a separator.
  assert.equal(
    store.resolveWebAuthFile({ PI_CODING_AGENT_DIR: "/srv/omp/agent" }),
    join(resolve("/srv/omp/agent"), store.WEB_AUTH_FILENAME),
  );
  assert.equal(store.resolveWebAuthFile({ OMP_WEB_AUTH_FILE: "/srv/creds.json" }), resolve("/srv/creds.json"));
