...
export interface ProjectInfo {
  ...
  isTopLevel: boolean;
  /** Origin repository label, or null when not found */
 ...
 * This avoids a child process when the file exists.
 */
function readOriginUrlFromConfigFile(configPath: string): string | null {
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
        ...
        inOrigin = /^remote\s+"origin"$/i.test(section);
        continue;
      }
      if (inOrigin) {
...
function readBranchFromHeadFile(gitDir: string): string | null {
  try {
    ...
    if (content.startsWith("ref: refs/heads/")) {
      const branch = content.slice("ref: refs/heads/".length).trim();
      return branch || null;
    }
  } catch {
    return null;
...
/**
 * Parse an origin repository label from a Git remote URL.
 * Returns an owner and repository path, or null when absent.
 */
export function parseRepositoryLabel(url?: string | null): string | null {
  if (typeof url !== "string") return null;
  const trimmed = url.trim();
  if (!trimmed || trimmed.startsWith("/") || trimmed.startsWith("file://")) return null;
  ...
  const normalized = cleanPath.replace(/^\/+/, "").replace(/\/+$/, "").replace(/\.git$/i, "");
...
function inferRemovedWorktree(cwd: string): ProjectInfo | null {
  ...
  if (!repoRoot || !existsSync(gitDir)) return null;
  const originUrl = readOriginUrlFromConfigFile(join(gitDir, "config"));
  const repositoryLabel = parseRepositoryLabel(originUrl);
  return { projectRoot: repoRoot, branch: basename(cwd), isWorktree: true, isTopLevel: true, repositoryLabel };
}
...
export async function resolveProject(cwd: string): Promise<ProjectInfo> {
  ...
  try {
    if (!existsSync(cwd)) {
      info = inferRemovedWorktree(cwd) ?? { projectRoot: cwd, branch: null, isWorktree: false, isTopLevel: false, repositoryLabel: null };
    ...
    let originUrl = readOriginUrlFromConfigFile(join(commonDir, "config"));
    if (!originUrl) {
      try {
        const outUrl = await git(cwd, ["config", "--get", "remote.origin.url"]);
        if (outUrl) originUrl = outUrl;
      } catch {
        originUrl = null;
      }
    }
    ...
    info = { projectRoot: cwd, branch: null, isWorktree: false, isTopLevel: false, repositoryLabel: null };
  }
