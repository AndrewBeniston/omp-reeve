...
test("maximising fills the workspace, where dragging keeps a chat column", () => {
  const window = { viewportWidth: 1366, sidebarOpen: true, sidebarWidth: 260 };

  assert.equal(getMaximisedRightPanelWidth(window), 1106);
  assert.equal(getRightPanelMaxWidth(window), 754);

  // A hidden sidebar gives its width to the panel as well.
  assert.equal(getMaximisedRightPanelWidth({ ...window, sidebarOpen: false }), 1366);
});
