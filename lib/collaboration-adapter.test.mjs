...
function makeHarness() {
  ...
  const adapter = new CollaborationAdapter({
    ...
    listHosts: async () => [
      { sessionId: "session-1", sessionName: "Review", participants: 3, access: "write", cwd: "/repo" },
    ],
...
test("lists active local hosts without starting one", async () => {
  const { adapter, starts } = makeHarness();
  const result = await adapter.execute("list");

  assert.equal(starts.length, 0);
  assert.equal(result.message, "1 active local Collab host\n- Review (session-1): 2 guests, write, /repo");
});
