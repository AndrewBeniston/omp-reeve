...
const { buildModelSelectorState, filterModelOptions, INITIAL_MODEL_MENU_STATE, reduceModelMenuState, resetModelOverride } = await createJiti(import.meta.url).import("./index.ts");
const { modelDisplayName } = await createJiti(import.meta.url).import("../model-display-name.ts");
...
test("shows every registry model when no enabled scope is configured", () => {
  const state = buildModelSelectorState({
    registry: [
...
test("keeps good OMP names and improves raw model ids", () => {
  assert.equal(modelDisplayName({ id: "claude-sonnet-5", name: "Claude Sonnet 5" }), "Claude Sonnet 5");
  assert.equal(modelDisplayName({ id: "kimi-k2.5", name: "kimi-k2.5" }), "Kimi K2.5");
  assert.equal(modelDisplayName({ id: "o3-mini", name: "o3-mini" }), "O3 Mini");
  assert.equal(modelDisplayName({ id: "deepseek-flash", name: "deepseek-flash" }), "DeepSeek Flash");
});

test("disambiguates duplicate friendly names within one provider", () => {
  const state = buildModelSelectorState({
    registry: [
      { provider: "anthropic", id: "claude-haiku-4-5", name: "Claude Haiku 4.5", thinkingLevels: ["off"] },
      { provider: "anthropic", id: "claude-haiku-4-5-20251001", name: "Claude Haiku 4.5", thinkingLevels: ["off"] },
    ],
    roles: [],
    modelScopeConfigured: false,
  }, label);

  assert.deepEqual(state.models.map((option) => option.name), ["Claude Haiku 4.5 (claude-haiku-4-5)", "Claude Haiku 4.5 (claude-haiku-4-5-20251001)"]);
});

test("filters models by friendly name, id, and provider", () => {
  const options = [
    { provider: "anthropic", modelId: "claude-sonnet-5", name: "Claude Sonnet 5" },
    { provider: "opencode-go", modelId: "kimi-k2.5", name: "Kimi K2.5" },
  ];

  assert.deepEqual(filterModelOptions(options, "sonnet").map((option) => option.modelId), ["claude-sonnet-5"]);
  assert.deepEqual(filterModelOptions(options, "kimi-k").map((option) => option.modelId), ["kimi-k2.5"]);
  assert.deepEqual(filterModelOptions(options, "opencode-go").map((option) => option.modelId), ["kimi-k2.5"]);
});

test("marks the current model when it reports no effort level", () => {
  const state = buildModelSelectorState({
    registry: [{ provider: "openai", id: "o3-mini", name: "o3-mini", thinkingLevels: [] }],
    roles: [],
    currentModel: { provider: "openai", modelId: "o3-mini" },
    currentThinkingLevel: "auto",
    modelScopeConfigured: false,
  }, label);

  assert.equal(state.modelRowsByProvider[0]?.options[0]?.selected, true);
  assert.equal(state.modelRowsByProvider[0]?.options[0]?.name, "O3 Mini");
});
...
test("names every supported provider route and title-cases fallback routes", () => {
  const state = buildModelSelectorState({
    registry: [
      { provider: "anthropic", id: "claude", name: "Claude", thinkingLevels: [] },
      { provider: "google", id: "gemini", name: "Gemini", thinkingLevels: [] },
      { provider: "google-antigravity", id: "antigravity", name: "Antigravity", thinkingLevels: [] },
      { provider: "openai", id: "gpt", name: "GPT", thinkingLevels: [] },
      { provider: "openai-codex", id: "gpt-codex", name: "GPT Codex", thinkingLevels: [] },
      { provider: "opencode-go", id: "kimi", name: "Kimi", thinkingLevels: [] },
      { provider: "vp0-local", id: "local", name: "Local", thinkingLevels: [] },
      { provider: "opencodex", id: "proxy", name: "Proxy", thinkingLevels: [] },
    ],
    roles: [],
    modelScopeConfigured: false,
  }, label);
