...
test("the window reports the start directory and the shell of its Terminal", () => {
...
test("the reply carries no shell identifier", () => {
  const reply = terminalReadReply([terminal({})], "terminal:1");
  assert.deepEqual(Object.keys(reply.value).sort(), ["attached", "shell", "startDir"]);
});

test("a window with several Terminals reads the one the human is looking at", () => {
  const terminals = [
    terminal({ tabId: "terminal:1", startDir: "/work/one", openedAt: 1000 }),
    terminal({ tabId: "terminal:2", startDir: "/work/two", openedAt: 2000 }),
  ];
  assert.equal(selectSessionTerminal(terminals, "terminal:1").startDir, "/work/one");
});
...
test("a window with no active Terminal reads the most recent one", () => {
  ...
  assert.equal(selectSessionTerminal(terminals, null).startDir, "/work/two");
  // A file Tab is active, so no Terminal is.
  assert.equal(selectSessionTerminal(terminals, "file:readme").startDir, "/work/two");
});

test("a shell that exited is not read", () => {
  const terminals = [
    terminal({ tabId: "terminal:1", startDir: "/work/one", live: false, openedAt: 2000 }),
    terminal({ tabId: "terminal:2", startDir: "/work/two", openedAt: 1000 }),
  ];
  assert.equal(selectSessionTerminal(terminals, "terminal:1").startDir, "/work/two");
});
...
test("the window answers the Terminal read control", () => {
  const reply = answerAgentControlRequest(request(TERMINAL_READ_CONTROL), [terminal({})], "terminal:1", true);
  assert.equal(reply.ok, true);
  assert.equal(reply.value.shell, "/bin/zsh");
});

test("a window with no Terminal surface stays silent", () => {
  // A plain browser tab owns no shell. An answer from it would hide the real
  // Terminal of a desktop window that shows the same Session.
  const reply = answerAgentControlRequest(request(TERMINAL_READ_CONTROL), [], null, false);
  assert.equal(reply, null);
});

test("a control this window does not serve answers unavailable", () => {
  const reply = answerAgentControlRequest(request("open_tab"), [terminal({})], "terminal:1", true);
...
test("the Terminal view reports its shell, its directory and its live state", async () => {
  ...
  assert.match(source, /startDir: opened\.cwd/);
