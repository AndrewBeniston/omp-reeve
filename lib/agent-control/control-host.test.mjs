...
test("the browser build registers no control", () => {
  // The host holds the one build gate. The channel holds none.
...
test("the Terminal read control takes no arguments", (t) => {
  const session = startSession(t);
  const control = session.tools.find((tool) => tool.name === TERMINAL_READ_TOOL_NAME);
  const schema = control.parameters.toJsonSchema();
  assert.deepEqual(Object.keys(schema.properties ?? {}), []);
});

test("a call that names another Session returns unavailable and changes nothing", async (t) => {
  // The Terminal read takes no Session id. The refusal stays in the channel
  // for a later control that accepts a task identifier.
  const session = startSession(t, {
    answer: () => ({ ok: true, value: { attached: true, startDir: "/work/project", shell: "/bin/zsh" } }),
  });
  const reply = await session.channel.call("terminal_read", { session: "session-b" });
...
test("a call that names its own Session is answered", async (t) => {
  ...
  const reply = await session.channel.call("terminal_read", { session: "session-a" });
