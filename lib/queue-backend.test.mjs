...
const { readSessionQueueFailures, recordQueueFailure, clearSessionQueueFailures } = await jiti.import("./queue-failure-store.ts");
...
test("a delivered queued message clears its persisted failure", async () => {
  const sessionId = "delivered-failure-session";
  let emit;
  const inner = makeInner({
    sessionId,
    subscribe: (listener) => {
  ...
  inner.agent.replaceQueues([], [
    { role: "user", content: [{ type: "text", text: "Delivered item" }], timestamp: 1 },
  ]);
  const wrapper = new AgentSessionWrapper(inner, makeEventBus());
  await wrapper.start();
  wrapper.queuePaused = true;
  ...
  assert.equal(readSessionQueueFailures(sessionId).length, 1);

  emit({
    type: "message_start",
    message: { role: "user", content: [{ type: "text", text: "Delivered item" }], timestamp: 1 },
  });

  assert.equal(readSessionQueueFailures(sessionId).length, 0);
  wrapper.destroy();
  clearSessionQueueFailures(sessionId);
});

test("Retry keeps saved file mentions with the recovered message", async () => {
  const sessionId = "retry-attachment-session";
  const inner = makeInner({ sessionId });
  const queues = { steering: [], followUp: [] };
  inner.agent.peekSteeringQueue = () => queues.steering;
  inner.agent.peekFollowUpQueue = () => queues.followUp;
  inner.agent.replaceQueues = (steering, followUp) => {
    queues.steering = [...steering];
    queues.followUp = [...followUp];
  };
  inner.agent.prepareQueuedMessages = async () => ({ commit: () => [] });
  inner.agent.steer = (message) => queues.steering.push(message);
  inner.agent.followUp = (message) => queues.followUp.push(message);
  inner.steer = async (text, images) => inner.agent.steer({
    role: "user",
  ...
  const fileMention = {
    role: "fileMention",
    files: [{ path: "/tmp/selected.txt", content: "Selected file content" }],
    timestamp: 1,
  };
  recordQueueFailure({
    id: "retry-with-file",
  ...
  const wrapper = new AgentSessionWrapper(inner, makeEventBus());
  await wrapper.send({ type: "retry_queue_item", id: "retry-with-file" });
  const [message] = queues.steering;
  const preparation = await inner.agent.prepareQueuedMessages([message], new AbortController().signal);

  assert.deepEqual(preparation.commit().map((item) => item.files[0].path), ["/tmp/selected.txt"]);
  wrapper.destroy();
  clearSessionQueueFailures(sessionId);
});
