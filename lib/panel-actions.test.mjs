...
const { isReopenableTabKind, stepTabIndex } = await jiti.import("./panel-actions.ts");
...
test("a closed Terminal is never worth reopening", () => {
  // Its shell ended with the Tab. Reopening one would be a dead shell wearing
  // a live one's name.
  assert.equal(isReopenableTabKind("terminal"), false);
  assert.equal(isReopenableTabKind("browser"), true);
  assert.equal(isReopenableTabKind("file"), true);
  assert.equal(isReopenableTabKind("sources"), true);
});
