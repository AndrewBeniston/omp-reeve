...
test("a corrupt Goal names its invalid field while ordinary Session state stays available", { timeout: 60_000 }, async () => {
  const { session, realSessionId } = await openSession();
  try {
    ...
    try {
      ...
      await assert.rejects(
        reopened.send({ type: "goal", op: "get" }),
        (error) => error.code === "goal_invalid_snapshot" && /tokensUsed/.test(error.message),
      );
    } finally {
      await reopened.shutdown();
    }
