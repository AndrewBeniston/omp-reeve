...
test("advertises browser-native fork and collaboration with canonical metadata", async () => {
  ...
  assert.equal(byName.get("collab")?.source, "builtin");
  assert.equal(byName.get("collab")?.description, "Share this session live via a relay");
  assert.equal(byName.get("collab")?.icon, "broadcast");
  assert.equal(byName.get("collab")?.subcommands?.map((entry) => entry.name).join(","), "view,list,status,stop");
...
test("advertises shared SDK builtins but not TUI-only commands", async () => {
  ...
  assert.equal(byName.has("collab"), true);
  assert.equal(byName.has("join"), false);
  assert.equal(byName.has("leave"), false);
});

test("every advertised SDK builtin has an executable browser path", async () => {
  const commands = await getAvailableSlashCommands(makeSession());
  const browserNative = new Set(["fork", "collab"]);
