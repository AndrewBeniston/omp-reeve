...
test("ranks an exact file stem before same-name directories", async () => {
  const { filterFileEntries } = await loadSubject();
  const result = filterFileEntries([
    { path: "docs/agents", isDir: true },
    { path: ".agents", isDir: true },
    { path: "AGENTS.md", isDir: false },
    { path: "docs/agents/domain.md", isDir: false },
  ], "AGENTS");

  assert.equal(result[0].path, "AGENTS.md");
  assert.equal(result[0].isDir, false);
});
