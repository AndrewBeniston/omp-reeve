...
export interface CollaborationQr {
  size: number;
  rows: string[];
  url: string;
}
...
export interface CollaborationSnapshot {
  ...
  mode: "write" | "view";
  browserUrl?: string;
  viewBrowserUrl?: string;
  terminalLink?: string;
  viewTerminalLink?: string;
...
export interface SlashCommandInfo {
  name: string;
  /** OMP's semantic slash-command icon name. */
  icon?: string;
  aliases?: string[];
  description?: string;
  input?: { hint: string };
...
export type ModelLike = Pick<Model<Api>, "id" | "provider" | "api" | "identity">;
