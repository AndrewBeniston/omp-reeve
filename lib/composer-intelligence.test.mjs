...
test("offers live OMP subagents through an OMP instruction", () => {
  const items = flattenSuggestionSections(buildAtMentionSections({
    query: "reviewer",
    files: [],
    skills: [],
  ...
  assert.equal(items[0].raw, "Use the live Reviewer Agent from OMP");
  assert.equal(items[0].detail, "Inspect the current changes");
  assert.equal(items[0].rightLabel, "Running");
});

test("builds every Reeve mention section from real sources with bounded Session context", () => {
  const sections = buildAtMentionSections({
    ...
    sessions: [{
      ...
      context: "A".repeat(20_000),
    }],
    tabs: [{ id: "tab-1", label: "Docs", detail: "https://example.com", kind: "browser" }],
    skills,
    plugins: [...plugins, {
    ...
    agents: [{ name: "reviewer", description: "Review a change", source: "project" }],
    subagents: [{
      id: "live-1",
      index: 0,
      agent: "Reviewer",
    ...
    mcpServers: [{ name: "linear", enabled: true, scope: "project" }],
  });
  ...
  const sessionRaw = items.find((item) => item.group === "sessions").raw;
  assert.equal(sessionRaw.slice(-8_000), "A".repeat(8_000));
  assert.equal(sessionRaw, `@session:session-1\n\nPrior work context:\n${"A".repeat(8_000)}`);
  assert.equal(items.find((item) => item.group === "mcp").raw, "@mcp:linear");
  assert.equal(items.find((item) => item.group === "agents").raw, "Use the Reviewer Agent from OMP");
  assert.equal(items.find((item) => item.group === "liveAgents").raw, "Use the live Reviewer Agent from OMP");
  assert.equal(items.find((item) => item.group === "tabs").raw, "@tab:tab-1");
  assert.equal(items.find((item) => item.id === "plugin:review-helper").rightLabel, "Tab for more");
});

test("caps ranked mention results at eight rows", () => {
  const items = flattenSuggestionSections(buildAtMentionSections({
    query: "result",
    files: Array.from({ length: 12 }, (_, index) => ({ path: `result-${index}.ts`, isDir: false })),
    skills: [],
    plugins: [],
  }));
...
test("uses separate file and connected-source queries for debounced mention search", () => {
  const sections = buildAtMentionSections({
    query: "review",
    fileQuery: "rev",
    connectedQuery: "",
    files: [{ path: "review.md", isDir: false }],
    sessions: [{ id: "session-1", label: "Review notes", detail: "Earlier work" }],
    skills: [{ ...skills[0], name: "review-helper", description: "Review code" }],
    plugins,
  });
  const items = flattenSuggestionSections(sections);
  assert.deepEqual(items.map((item) => item.kind), ["file", "session"]);

  const connected = flattenSuggestionSections(buildAtMentionSections({
    query: "review",
    fileQuery: "",
    connectedQuery: "rev",
    files: [{ path: "review.md", isDir: false }],
  ...
  assert.deepEqual(connected.map((item) => item.kind), ["skill"]);
});

test("formats the latest Session transcript into bounded plain context", () => {
  const context = formatSessionTranscriptContext([
  ...
  assert.equal(context.length, 8_000);
  assert.equal(context, "A".repeat(8_000));
});

test("attaches a bounded transcript when a Session suggestion is selected", () => {
  const [session] = flattenSuggestionSections(buildAtMentionSections({
    query: "",
    files: [],
    skills: [],
    plugins: [],
    sessions: [{ id: "session-1", label: "Prior work" }],
  }));
  const selected = attachSessionTranscriptContext(session, [{
    role: "assistant",
    content: [{ type: "text", text: "A".repeat(9_000) }],
  }]);

  assert.equal(selected.raw, `@session:session-1\n\nPrior work context:\n${"A".repeat(8_000)}`);
});

test("ranks real Session sources before transcript fetches", () => {
  const sessions = rankComposerSessionSources([
    { id: "one", label: "Older", detail: "Unrelated" },
    { id: "two", label: "Review notes", detail: "Parser work" },
  ], "review", 5);
  assert.deepEqual(sessions.map((session) => session.id), ["two"]);
});
...
test("Tab completes a Plugin mention row", () => {
  const plugin = flattenSuggestionSections(buildAtMentionSections({
    query: "review helper",
    files: [],
    skills: [],
    plugins: [{
  ...
  assert.equal(plugin.rightLabel, "Tab for more");
  assert.deepEqual(resolveAutocompleteSelection("Tab", false, [plugin], 0), {
    captured: true,
    item: plugin,
  });
});
