...
export async function attachSessionProjectInfo(sessions: SessionInfo[]): Promise<SessionInfo[]> {
  ...
  return sessions.map((session) => {
    ...
    return {
      ...
      ...(project?.branch ? { gitBranch: project.branch } : {}),
      ...(project?.repositoryLabel ? { repositoryLabel: project.repositoryLabel } : {}),
      ...(project !== undefined ? { isWorktree: project.isWorktree } : {}),
...
export function mergeSessionLists(
  ...
  // Disk identity remains authoritative once the JSONL exists. The live OMP
  // Session repairs metadata that its bounded disk scan could not reach.
  for (const session of persistedSessions) {
    const live = byId.get(session.id);
    const liveHasFirstMessage = live?.firstMessage && live.firstMessage !== "(no messages)";
    const persistedNeedsLiveMetadata = Boolean(
      live
      && (
        (!session.name && live.name)
        || live.messageCount > session.messageCount
        || (session.firstMessage === "(no messages)" && liveHasFirstMessage)
      ),
    );
    byId.set(session.id, persistedNeedsLiveMetadata ? {
      ...session,
      name: session.name ?? live?.name,
      messageCount: Math.max(session.messageCount, live?.messageCount ?? 0),
      firstMessage: session.firstMessage === "(no messages)" && liveHasFirstMessage
        ? live.firstMessage
...
const SESSION_LIST_FALLBACK_PREFIX_BYTES = 64 * 1024;

function firstUserMessageFromBoundedPrefix(filePath: string, fileSize: number): string | undefined {
  let fd: number;
  try {
  ...
  try {
    const buffer = Buffer.allocUnsafe(Math.min(fileSize, SESSION_LIST_FALLBACK_PREFIX_BYTES));
    ...
    for (const line of content.split(/\r?\n/)) {
      ...
      try {
        ...
        // A truncated or malformed line cannot supply a safe title fallback.
      }
    }
    return undefined;
  } finally {
...
async function loadAllSessions(): Promise<SessionInfo[]> {
  ...
  const sessions = ompSessions.map((s) => {
    ...
    const recoveredFirstMessage = (!s.firstMessage || s.firstMessage === "(no messages)") && s.size > 4096
      ? firstUserMessageFromBoundedPrefix(s.path, s.size)
      : undefined;
    return {
      path: s.path,
      id: s.id,
      ...
      messageCount: recoveredFirstMessage ? Math.max(1, s.messageCount) : s.messageCount,
      firstMessage: (recoveredFirstMessage ?? s.firstMessage) || "(no messages)",
