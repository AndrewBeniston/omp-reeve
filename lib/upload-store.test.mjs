...
const { storeBrowserUpload, findBrowserUpload, MAX_UPLOAD_FILE_BYTES } = await createJiti(import.meta.url).import("./upload-store.ts");
...
test("the HTTP route accepts raw bytes and never returns a server path", async (t) => {
  ...
  const { GET } = await jiti.import("../app/api/sessions/[id]/uploads/[uploadId]/route.ts");
...
test("the browser upload call sends the original file and keeps desktop paths separate", async (t) => {
  ...
  const { uploadBrowserFile, addBrowserUpload, selectedAttachmentPaths, addComposerAttachments } = await jiti.import("./composer-attachment-state.ts");
