...
import { mkdtemp, readdir, rm, writeFile, mkdir, readFile, truncate, utimes } from "node:fs/promises";
...
test("startup repairs crashes before and after metadata commit", async (t) => {
  ...
  const stagingDirectory = join(root, ".upload-crash");
  await mkdir(stagingDirectory);
  await writeFile(join(stagingDirectory, "payload.blob"), "unfinished");
  const old = new Date(Date.now() - 20_000);
  await utimes(stagingDirectory, old, old);
...
test("damaged records and size mismatches do not block later uploads", async (t) => {
  const root = await fixture(t);
  const healthy = await storeBrowserUpload({
    root, sessionId: sessionA, key, name: "healthy.txt", mediaType: "text/plain", size: 5, body: bytes("hello"),
  });
  const mismatched = await storeBrowserUpload({
    root, sessionId: sessionA, key: "cd000000-0000-4000-8000-000000000002",
    name: "mismatch.txt", mediaType: "text/plain", size: 5, body: bytes("world"),
  });
  const damagedId = `up_${"c".repeat(32)}`;
  await writeFile(join(root, sessionA, `${mismatched.id}.blob`), "x");
  await writeFile(join(root, sessionA, `${damagedId}.blob`), "damaged");
  await writeFile(join(root, sessionA, `${damagedId}.json`), "{");
  const warnings = captureWarnings(t);

  const next = await storeBrowserUpload({
    root, sessionId: sessionA, key: "cd000000-0000-4000-8000-000000000003",
    name: "next.txt", mediaType: "text/plain", size: 4, body: bytes("next"),
  });

  assert.ok(await findBrowserUpload({ root, sessionId: sessionA, id: healthy.id }));
  assert.ok(await findBrowserUpload({ root, sessionId: sessionA, id: next.id }));
  assert.deepEqual([...(await readdir(join(root, sessionA)))].sort(), [
    `${healthy.id}.blob`, `${healthy.id}.json`, `${next.id}.blob`, `${next.id}.json`,
  ]);
  assert.equal(warnings.length, 2);
});

test("a slow upload body does not lock Session metadata", async (t) => {
  const root = await fixture(t);
  let releaseBody;
  let markStarted;
  const bodyGate = new Promise(resolve => { releaseBody = resolve; });
  const bodyStarted = new Promise(resolve => { markStarted = resolve; });
  ...
  const firstPromise = storeBrowserUpload({
    root, sessionId: sessionA, key, name: "slow.txt", mediaType: "text/plain", size: 4, body: slowBody,
  });
  await bodyStarted;
  ...
  let secondError;
  try {
    second = await storeBrowserUpload({
      root, sessionId: sessionB, key: "cd000000-0000-4000-8000-000000000002",
      name: "fast.txt", mediaType: "text/plain", size: 4, body: bytes("fast"),
    });
  } catch (error) {
    secondError = error;
  } finally {
  ...
  assert.ok(await findBrowserUpload({ root, sessionId: sessionA, id: first.id }));
  assert.ok(await findBrowserUpload({ root, sessionId: sessionB, id: second.id }));
});
...
test("server registration logs upload cleanup failures and continues", async (t) => {
  const agentDir = await fixture(t);
  const { getAgentDir, setAgentDir } = await import("@oh-my-pi/pi-utils/dirs");
  const previousAgentDir = getAgentDir();
  const previousEnv = process.env.PI_CODING_AGENT_DIR;
  ...
  await writeFile(join(agentDir, "browser-uploads"), "not a directory");
  const warnings = captureWarnings(t);
  const { register } = await createJiti(import.meta.url, { alias: { "@": process.cwd() } }).import("../instrumentation.ts");
  ...
  assert.match(String(warnings[0][0]), /upload/i);
});

test("a Session reads only its own uploaded bytes and leaves the draft releasable", async (t) => {
  const root = await fixture(t);
  ...
  assert.equal(await releaseBrowserUpload({ root, sessionId: sessionA, id: upload.id }), true);
});
...
test("the OMP prompt receives browser upload context before the user's text", async (t) => {
  ...
  const inner = {
    ...
    prompt: async message => { received.push(["prompt", message]); return true; },
...
test("a prompt rejected during a response leaves its browser upload releasable", async (t) => {
  const agentDir = await fixture(t);
  const { getAgentDir, setAgentDir } = await import("@oh-my-pi/pi-utils/dirs");
  const previousAgentDir = getAgentDir();
  const previousEnv = process.env.PI_CODING_AGENT_DIR;
  ...
  const upload = await storeBrowserUpload({ sessionId: sessionA, key, name: "notes.txt", mediaType: "text/plain", size: 5, body: bytes("hello") });
  const inner = {
    sessionId: sessionA, isStreaming: true, isBashRunning: false,
    settings: { get: () => undefined },
    agent: { state: {} },
  ...
  await assert.rejects(
    wrapper.send({ type: "prompt", message: "Review this file", uploads: [upload.id] }),
    { name: "AttachmentPathError", status: 400 },
  );
  assert.equal(await releaseBrowserUpload({ sessionId: sessionA, id: upload.id }), true);
});
