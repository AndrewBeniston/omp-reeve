...
const { readDefaultModelRole } = await createJiti(import.meta.url)
  .import("./model-roles.ts");

async function withSettings(run, inMemory = true) {
  ...
  try {
    await run({ settings: await Settings.loadIsolated({ cwd, agentDir, inMemory }), cwd, agentDir });
  } finally {
    await rm(root, { recursive: true, force: true });
  }
}

test("reload preserves the provider when API and subscription routes share a model id", async () => {
  await withSettings(async ({ settings, cwd, agentDir }) => {
    for (const provider of ["openai", "openai-codex"]) {
      const model = { provider, modelId: "gpt-example" };
      await persistExplicitStartupPreferences(
        settings,
        { model },
        { model, thinkingLevel: "medium", supportsThinking: true },
      );
      const reloaded = await Settings.loadIsolated({ cwd, agentDir });
      assert.deepEqual(readDefaultModelRole(reloaded), model);
    }
  }, false);
});
...
test("persists Auto as the configured thinking selector", async () => {
  await withSettings(async ({ settings }) => {
    await persistExplicitStartupPreferences(
      settings,
      { thinkingLevel: "auto" },
