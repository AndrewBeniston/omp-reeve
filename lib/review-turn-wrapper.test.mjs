...
async function wrapperScenarios() {
  ...
  const script = `
    ...
    // Destroyed while the baseline is being taken.
    {
...
test("the Session wrapper records a prompt's lifecycle without losing or misplacing one", { timeout: 120_000 }, async () => {
  ...
  // Destroyed mid-baseline: the run is never dispatched, and the span it had
  // already opened is closed rather than left running for ever.
