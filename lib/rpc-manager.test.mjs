...
test("RPC session startup gates untrusted project code before creating the session", async () => {
  ...
  // The gated options reach the session options. The local name may change, so
  // the test follows the spread back to the trust gate instead of one spelling.
  const spread = startupSource.match(/\.\.\.\((\w+) \?\? \{\}\)/);
  assert.ok(spread, "the gated project options never reach the session options");
  const gatedName = spread[1];
  const declaration = startupSource.match(new RegExp(`const ${gatedName}[^=]*=([\\s\\S]*?);\n`));
  assert.ok(
    gatedName === "untrusted" || (declaration !== null && /\buntrusted\b/.test(declaration[1])),
    "the spread session options do not come from untrustedProjectSessionOptions",
  );
  const spreadIndex = startupSource.indexOf(spread[0]);
  assert.ok(spreadIndex > gateIndex);
  assert.ok(spreadIndex < createIndex);
});
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /await getRpcSession\(header\?\.id \?\? id\)\?\.shutdown\(\)/);
...
test("RPC Stop pauses steering and follow-up messages without auto-resume", async () => {
  ...
  assert.deepEqual(abortOptions, { reason: "Interrupted by user", goalReason: "interrupted" });
  const state = await wrapper.send({ type: "get_state" });
