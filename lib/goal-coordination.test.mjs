...
const { GoalApiError } = await jiti.import("./goal-command.ts");
...
test("startup disposes the AgentSession when Goal reconciliation fails", { timeout: 60_000 }, async () => {
  ...
  const originalReconcile = AgentSessionWrapper.prototype.reconcileGoalState;
  let disposeCalls = 0;
  AgentSession.prototype.dispose = function (...args) {
    disposeCalls += 1;
    return originalDispose.apply(this, args);
  };
  AgentSessionWrapper.prototype.reconcileGoalState = async function () {
    throw new GoalApiError("goal_unsupported", "Goal reconciliation failed for this test.", 501);
  };
  try {
    const cwd = await mkdtemp(join(root, "project-"));
    await assert.rejects(
      startRpcSession("new", "", cwd),
      (error) => error.code === "goal_unsupported" && error.status === 501,
    );
    assert.equal(disposeCalls, 1);
  } finally {
    AgentSessionWrapper.prototype.reconcileGoalState = originalReconcile;
