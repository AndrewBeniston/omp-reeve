...
function speechServices({ cached = true, download = async () => {}, createController } = {}) {
  ...
  const services = {
    ...
    download,
    createController: createController ?? function ({ editor, onStateChange, onAudio }) {
      const controller = {
        state: "idle",
        disposed: false,
        async toggle() {
          if (this.state === "idle") {
            ...
            onAudio(Float32Array.from([0.25, -0.25]));
          } else {
...
test("a dictation publishes OMP states and its final text", async () => {
  ...
  assert.equal(bridge.recording.mimeType, "audio/wav");
  assert.equal(bridge.recording.bytes.byteLength, 48);
  await bridge.close();
});
...
test("the speech route opens the retained failed recording as a WAV", async () => {
  const { cacheSessionPath, invalidateSessionPathCache } = await import("./session-reader.ts");
  const { GET } = await import("../app/api/sessions/[id]/speech/route.ts");
  const directory = mkdtempSync(join(tmpdir(), "reeve-speech-recording-"));
  const sessionId = "speech-recording-test";
  const file = join(directory, "session.jsonl");
  writeFileSync(file, JSON.stringify({ type: "session", version: 3, id: sessionId, timestamp: new Date().toISOString(), cwd: directory }) + "\n");
  cacheSessionPath(sessionId, file);
  const { services } = speechServices({
    createController({ editor, onStateChange, onWarning, onAudio }) {
      return {
        state: "idle",
        async toggle() {
          if (this.state === "idle") { this.state = "recording"; onStateChange("recording"); onAudio(Float32Array.from([0.5])); }
          else { this.state = "transcribing"; onStateChange("transcribing"); editor.commitVolatileText("note"); onWarning("Transcription failed"); this.state = "idle"; onStateChange("idle"); }
        },
        dispose() {},
      };
    },
  ...
  try {
    await bridge.start();
    await bridge.stop();
    const response = await GET(new Request(`http://localhost:30141/api/sessions/${sessionId}/speech?recording=1`, { headers: { host: "localhost:30141" } }), { params: Promise.resolve({ id: sessionId }) });
    assert.equal(response.status, 200);
