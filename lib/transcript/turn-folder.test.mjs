...
test("Session entries alone restore the recorded Session in source order", () => {
  ...
  // OMP 18.2.11 persists steering, but not the follow-up queue marker.
  // Reeve must not infer that this loaded message interrupted the first Turn.
  assert.deepEqual(contents(turns), [
...
test("a live Turn uses the user timestamp when agent_start has no timestamp", () => {
  const startedAt = Date.parse("2026-09-23T10:00:01.000Z");
  const turn = foldTurns([
    { type: "agent_start" },
    { type: "message", timestamp: startedAt, message: { role: "user", content: "Question" } },
  ])[0];
  ...
  assert.equal(shouldTickTurnClock(turn), true);
  assert.equal(getTurnElapsedMs(turn, startedAt + 1_000), 1_000);
});
...
test("a saved aborted assistant message marks its Turn as interrupted", () => {
  const turn = foldTurns([
    { type: "message", message: { role: "user", content: "Question" } },
    {
      type: "message",
      message: {
        role: "assistant",
        content: [{ type: "toolCall", toolCallId: "call-1", toolName: "read", input: { path: "README.md" } }],
        stopReason: "toolUse",
      },
    },
  ...
  assert.equal(turn.status, "stopped");
  assert.equal(turn.stopSource, "process");
  assert.equal(turn.items.length, 3);
});

test("a saved process stop ends the clock at the last entry before the abort marker", () => {
  const turn = foldTurns([
    { type: "message", timestamp: "2026-09-23T10:00:00.000Z", message: { role: "user", content: "Question" } },
    {
      type: "message",
    ...
    { type: "message", timestamp: "2026-09-23T10:00:02.000Z", message: { role: "custom", customType: "tool_execution_start", content: "", display: false } },
    { type: "message", timestamp: "2026-09-23T10:00:03.000Z", message: { role: "custom", customType: "session_exit", content: "", display: false } },
    {
      type: "message",
      timestamp: "2026-09-23T10:00:44.000Z",
      message: { role: "assistant", content: [], stopReason: "aborted" },
    },
  ])[0];

  assert.equal(turn.completedAt, Date.parse("2026-09-23T10:00:02.000Z"));
  assert.equal(getTurnElapsedMs(turn, Date.parse("2026-09-23T10:00:44.000Z")), 2_000);
});

test("a saved process stop without a process entry has no known end time", () => {
  const turn = foldTurns([
    { type: "message", timestamp: "2026-09-23T10:00:00.000Z", message: { role: "user", content: "Question" } },
    { type: "message", timestamp: "2026-09-23T10:00:44.000Z", message: { role: "assistant", content: [], stopReason: "aborted" } },
  ])[0];
...
test("persisted and live approval denials increment their Turn count once", () => {
  const user = { type: "message", id: "user-1", message: { role: "user", content: "Question" } };
  const call = {
    type: "message",
    id: "call-1",
    message: {
      role: "assistant",
      content: [{ type: "toolCall", toolCallId: "tool-1", toolName: "write", input: {} }],
    },
  };
  const denied = {
    role: "toolResult",
    toolCallId: "tool-1",
    toolName: "write",
    content: [{ type: "text", text: "Tool call denied by user: write" }],
    isError: true,
  };
  const persisted = foldTurns([
    user,
    call,
    { type: "message", id: "result-1", message: denied },
  ]);
  const live = foldTurns([
    { ...user, type: "message_end" },
    { ...call, type: "message_end" },
    { type: "message_start", message: denied },
    { type: "message_end", message: denied },
  ]);

  assert.equal(persisted[0].deniedActionCount, 1);
  assert.equal(live[0].deniedActionCount, 1);
});

test("ordinary tool errors do not increment the denied-action count", () => {
  const turn = foldTurns([
    { type: "message", message: { role: "user", content: "Question" } },
    {
      type: "message",
      message: {
        role: "toolResult",
        content: [{ type: "text", text: "Error: Access denied by the file system" }],
        isError: true,
      },
    },
  ])[0];

  assert.equal(turn.deniedActionCount, 0);
});
