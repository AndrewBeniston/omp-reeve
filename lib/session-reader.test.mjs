...
test("attaches each saved file mention to the user message before it", () => {
  const fileMentionEntry = (id, parentId) => ({
    type: "message",
    id,
    parentId,
  ...
  const context = buildSessionContext([
    ...
    fileMentionEntry("f1", "u1"),
    assistantEntry("a1", "f1", "first answer"),
    userEntry("u2", "a1", "second request"),
  ]);

  assert.deepEqual(context.messages.map((message) => message.attachments?.map(({ name }) => name)), [
    ["a.ts"],
    undefined,
    undefined,
  ]);
...
test("classifies a saved usage-limit failure on the server", () => {
  const context = buildSessionContext([
  ...
  assert.deepEqual(context.messages[1].usageLimit, { retryAfterMs: 2000 });
});

test("restores default model-change notes at their persisted transcript positions", () => {
  const modelChange = (id, parentId, model, fields = {}) => ({
    type: "model_change",
    id,
    parentId,
    timestamp: "2026-01-01T00:00:00.000Z",
  ...
  const entries = [
    modelChange("m1", null, "openai/model-a"),
    userEntry("u1", "m1", "First question"),
    assistantEntry("a1", "u1", "First answer"),
    modelChange("role-model", "a1", "anthropic/plan", { role: "plan" }),
    modelChange("fallback", "role-model", "openai/model-fallback", { resolvedModelIsFallback: true }),
    modelChange("m2", "fallback", "anthropic/model-b"),
    userEntry("u2", "m2", "Second question"),
    assistantEntry("a2", "u2", "Second answer"),
    modelChange("m3", "a2", "google/model-c"),
  ];

  const context = buildSessionContext(entries);

  assert.deepEqual(context.modelChanges, [
    { entryId: "m2", position: 2, fromModel: "openai/model-a", toModel: "anthropic/model-b" },
    { entryId: "m3", position: 4, fromModel: "anthropic/model-b", toModel: "google/model-c" },
  ]);
  assert.deepEqual(context.fallbackRoutes, [
    { entryId: "fallback", position: 2, toModel: "openai/model-fallback" },
  ]);
});

test("ignores model changes that are not fallback routes", () => {
  const entries = [
    {
      type: "model_change",
      id: "not-fallback",
  ...
  assert.deepEqual(buildSessionContext(entries).fallbackRoutes, []);
});
...
test("preserves root custom entries as hidden transcript markers", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      type: "custom",
  ...
  assert.deepEqual(context.entryIds, ["u1", "exit-1"]);
