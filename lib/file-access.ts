...
// The home directory is read through a function so that the build-time file
// tracer (@vercel/nft) cannot fold it into a constant. When it can, it treats
// readdirSync(<constant>) as an asset directory and globs the whole home tree
// at build time. On a Windows runner that walk hits the SendTo junction and the
  ...
  try {
    ...
    for (const name of readdirSync(home)) {
      if (/^omp-cwd-\d{8}$/.test(name)) {
        roots.add(normalizeSlashes(path.join(home, name)));
      }
    }
  } catch {
