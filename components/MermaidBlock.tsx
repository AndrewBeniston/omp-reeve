...
// The syntax highlighter requires vendor style maps through its renderer API.
const CODE_LINE_NUMBER_STYLE: CSSProperties = { color: "var(--ui-text-dim)", fontStyle: "normal" };
const CODE_BLOCK_STYLE: CSSProperties = {
  ...
  lineHeight: 1.62,
  borderRadius: 0,
  background: "color-mix(in srgb, var(--ui-main) 92%, var(--ui-surface-elevated))",
};
...
function MermaidZoomDialog({ svg, onClose }: { svg: string; onClose: () => void }) {
  ...
  return (
    <dialog
      ref={dialogRef}
      className={`mermaid-zoom-dialog ${styles.mermaidZoomDialog}`}
    ...
    <div className="markdown-code-block">
      ...
      {isStreaming ? (
        <pre className={styles.streamingCode}>
          <code className={styles.streamingCodeContent}>{code}</code>
        </pre>
      ) : (
        /* DESIGN.md permits the vendor syntax-highlight color map. */
        <SyntaxHighlighter
          language={lang || "text"}
