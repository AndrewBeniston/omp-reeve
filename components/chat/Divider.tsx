...
export function Divider({ turnId, status, startedAt, completedAt, previousMessageCount, deniedActionCount = 0, forceExpanded = false, now: suppliedNow, children }: DividerProps) {
  ...
  const toggle = () => {
    ...
    const next = !expanded;
    if (next && dividerRef.current && triggerRef.current) {
  ...
  return <div ref={dividerRef} className={styles.divider} data-transcript-divider>
    <button ref={triggerRef} type="button" className={styles.trigger} aria-expanded={expanded} onClick={toggle} title={deniedLabel ? t("transcript.divider.deniedActionTooltip", { count: deniedActionCount }) : undefined}>
