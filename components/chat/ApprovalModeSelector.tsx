"use client";

import { useEffect, useRef, useState, type ReactNode } from "react";
import { useI18n } from "@/hooks/useI18n";
import { APPROVAL_MODES, type ApprovalMode } from "@/lib/approval-mode";
import { openExternal } from "@/lib/open-external";
import { Menu, MenuItem } from "../ui/Menu";
import { Tooltip } from "../ui/Tooltip";
import styles from "./approval-mode-selector.module.css";

const MODE_LABELS: Record<ApprovalMode, string> = {
  "always-ask": "approvalMode.ask",
  write: "approvalMode.write",
  yolo: "approvalMode.full",
};

const MODE_DESCRIPTIONS: Record<ApprovalMode, string> = {
  "always-ask": "approvalMode.askDescription",
  write: "approvalMode.writeDescription",
  yolo: "approvalMode.fullDescription",
};

export function ApprovalModeSelector({
  mode,
  changing,
  error,
  restricted,
  onChange,
  onTrust,
}: {
  mode: ApprovalMode | null;
  changing: boolean;
  error: string | null;
  restricted: boolean;
  onChange: (mode: ApprovalMode) => void;
  onTrust: () => void;
}) {
  const { t } = useI18n();
  const [open, setOpen] = useState(false);
  const rootRef = useRef<HTMLDivElement>(null);
  const triggerRef = useRef<HTMLButtonElement>(null);

  useEffect(() => {
    if (!open) return;
    const close = (event: MouseEvent) => {
      if (!(event.target instanceof Node) || rootRef.current?.contains(event.target)) return;
      setOpen(false);
    };
    document.addEventListener("mousedown", close);
    return () => document.removeEventListener("mousedown", close);
  }, [open]);

  if (restricted) {
    return (
      <button
        type="button"
        aria-label={t("trust.resourcesNotLoaded")}
        title={t("trust.resourcesNotLoaded")}
        className={styles.trigger}
        onClick={onTrust}
      >
        <ModeIcon mode="restricted" />
        <span>{t("trust.resourcesNotLoaded")}</span>
      </button>
    );
  }

  if (!mode) return null;

  return (
    <div ref={rootRef} className={styles.root}>
      <Tooltip content={t("approvalMode.changePermissions")}>
        <button
          ref={triggerRef}
          type="button"
          aria-label={t(MODE_LABELS[mode])}
          aria-haspopup="menu"
          aria-expanded={open}
          data-approval-mode={mode}
          disabled={changing}
          className={styles.trigger}
          onClick={() => setOpen((current) => !current)}
        >
          <ModeIcon mode={mode} />
          <span>{t(MODE_LABELS[mode])}</span>
          <ChevronIcon />
        </button>
      </Tooltip>
      <Menu
        open={open}
        label={t("approvalMode.menu")}
        onClose={() => setOpen(false)}
        triggerRef={triggerRef}
        className={styles.menu}
      >
        <div className={styles.menuHeader}>
          <span>{t("approvalMode.title")}</span>
          <button
            type="button"
            className={styles.learnMore}
            onClick={() => openExternal("https://omp.sh/")}
          >
            {t("approvalMode.learnMore")}
          </button>
        </div>
        {APPROVAL_MODES.map((option) => (
          <MenuItem
            key={option}
            role="menuitemradio"
            checked={mode === option}
            data-approval-mode={option}
            disabled={changing}
            icon={<ModeIcon mode={option} />}
            className={styles.option}
            onClick={() => {
              setOpen(false);
              if (mode !== option) onChange(option);
            }}
          >
            <span className={styles.optionText}>
              <span className={styles.optionLabel}>{t(MODE_LABELS[option])}</span>
              <span className={styles.optionDescription}>{t(MODE_DESCRIPTIONS[option])}</span>
            </span>
            {mode === option ? <CheckIcon /> : null}
          </MenuItem>
        ))}
        {error ? <div role="alert" className={styles.error}>{error}</div> : null}
      </Menu>
      {error && !open ? <div role="alert" className={styles.srError}>{error}</div> : null}
    </div>
  );
}

function SvgIcon({ children }: { children: ReactNode }) {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill="none" aria-hidden="true">
      {children}
    </svg>
  );
}

function ModeIcon({ mode }: { mode: ApprovalMode | "restricted" }) {
  if (mode === "always-ask") {
    return <SvgIcon><path fill="currentColor" d="M12.6683 4.16699C12.6683 3.84391 12.4065 3.58203 12.0834 3.58203C11.7603 3.58203 11.4984 3.84391 11.4984 4.16699V7.91699L11.4847 8.05078C11.4227 8.35375 11.1547 8.58203 10.8334 8.58203C10.4662 8.58203 10.1685 8.28411 10.1683 7.91699V3.75C10.1683 3.42691 9.90646 3.16504 9.58337 3.16504C9.26029 3.16504 8.99841 3.42691 8.99841 3.75V7.91699C8.99824 8.28411 8.70053 8.58203 8.33337 8.58203C7.96621 8.58203 7.66851 8.28411 7.66833 7.91699V5C7.66833 4.67691 7.40646 4.41504 7.08337 4.41504C6.76029 4.41504 6.49841 4.67691 6.49841 5V9.30371C6.53326 9.3429 6.56715 9.38359 6.59998 9.42578L8.02478 11.2588C8.25005 11.5486 8.19821 11.9659 7.90857 12.1914C7.6187 12.4169 7.20048 12.365 6.97498 12.0752L5.55017 10.2432C5.15812 9.7391 4.41813 9.73637 4.01501 10.1924C4.04396 10.426 4.11486 10.8323 4.25525 11.3486C4.44664 12.0525 4.75404 12.9113 5.21619 13.7383C6.14103 15.3931 7.62465 16.835 10.0004 16.835C12.8545 16.8348 15.1682 14.5211 15.1683 11.667V6.25C15.1683 5.92691 14.9065 5.66504 14.5834 5.66504C14.2603 5.66504 13.9984 5.92691 13.9984 6.25V9.16699C13.9982 9.53411 13.7005 9.83203 13.3334 9.83203C12.9662 9.83203 12.6685 9.53411 12.6683 9.16699V4.16699ZM13.9984 4.42578C14.1828 4.36671 14.3794 4.33496 14.5834 4.33496C15.641 4.33496 16.4984 5.19237 16.4984 6.25V11.667C16.4982 15.2557 13.589 18.1649 10.0004 18.165C6.95953 18.165 5.10939 16.2734 4.05505 14.3867C3.52774 13.4431 3.1843 12.4787 2.97205 11.6982C2.76447 10.9349 2.66834 10.2954 2.66833 10C2.66833 9.87959 2.70117 9.76148 2.76306 9.6582C3.28988 8.78018 4.26555 8.40372 5.16833 8.56152V5C5.16833 3.94237 6.02575 3.08496 7.08337 3.08496C7.31706 3.08496 7.54039 3.12845 7.74744 3.20508C7.98218 2.41297 8.7151 1.83496 9.58337 1.83496C10.1836 1.83496 10.7186 2.11176 11.0697 2.54395C11.3639 2.35978 11.7107 2.25195 12.0834 2.25195C13.141 2.25195 13.9984 3.10937 13.9984 4.16699V4.42578Z" /></SvgIcon>;
  }
  if (mode === "write") {
    return <SvgIcon><path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d="M9.06543 1.95123C9.66107 1.69076 10.3389 1.69071 10.9346 1.95123L15.9346 4.13873C16.7832 4.51008 17.3311 5.34917 17.3311 6.27545V10.5528C17.3309 14.6017 14.0489 17.8847 10 17.8848C5.95108 17.8846 2.66813 14.6017 2.66797 10.5528V6.27545C2.66797 5.34924 3.21695 4.51012 4.06543 4.13873L9.06543 1.95123ZM10.4014 3.16998C10.1456 3.05814 9.85444 3.05819 9.59863 3.16998L4.59863 5.35748C4.23427 5.51708 3.99805 5.87764 3.99805 6.27545V10.5528C3.99821 13.8671 6.68563 16.5546 10 16.5547C13.3144 16.5546 16.0008 13.8671 16.001 10.5528V6.27545C16.001 5.87756 15.7658 5.51703 15.4014 5.35748L10.4014 3.16998Z" /><path fill="currentColor" d="M13.4678 11.4318L13.333 11.4182H10.833C10.466 11.4183 10.1682 11.7162 10.168 12.0832C10.168 12.4504 10.4659 12.7481 10.833 12.7482H13.333L13.4678 12.7346C13.7706 12.6724 13.9981 12.4044 13.9981 12.0832C13.9979 11.7621 13.7706 11.494 13.4678 11.4318ZM7.65336 12.426C7.46431 12.7406 7.05607 12.8424 6.74125 12.6535C6.42646 12.4646 6.32395 12.0563 6.51274 11.7414L7.55668 10.0002L6.51274 8.25899C6.32395 7.94412 6.42646 7.53583 6.74125 7.34688C7.05607 7.15799 7.46431 7.25975 7.65336 7.57442L8.90336 9.6584C9.0296 9.86893 9.0296 10.1315 8.90336 10.342L7.65336 12.426Z" /></SvgIcon>;
  }
  return <SvgIcon><path fill="currentColor" fillRule="evenodd" clipRule="evenodd" d="M9.06543 1.95123C9.66107 1.69076 10.3389 1.69071 10.9346 1.95123L15.9346 4.13873C16.7832 4.51008 17.3311 5.34917 17.3311 6.27545V10.5528C17.3309 14.6017 14.0489 17.8847 10 17.8848C5.95108 17.8846 2.66813 14.6017 2.66797 10.5528V6.27545C2.66797 5.34924 3.21695 4.51012 4.06543 4.13873L9.06543 1.95123ZM10.4014 3.16998C10.1456 3.05814 9.85444 3.05819 9.59863 3.16998L4.59863 5.35748C4.23427 5.51708 3.99805 5.87764 3.99805 6.27545V10.5528C3.99821 13.8671 6.68563 16.5546 10 16.5547C13.3144 16.5546 16.0008 13.8671 16.001 10.5528V6.27545C16.001 5.87756 15.7658 5.51703 15.4014 5.35748L10.4014 3.16998Z" /><path fill="currentColor" d="M10.8883 13.1116C10.8883 13.6025 10.4903 14.0005 9.99936 14.0005C9.50844 14.0005 9.11047 13.6025 9.11047 13.1116C9.11047 12.6207 9.50844 12.2227 9.99936 12.2227C10.4903 12.2227 10.8883 12.6207 10.8883 13.1116ZM10.5169 10.8949L11.1135 7.31519C11.2283 6.62672 10.6974 6 9.99941 6C9.30145 6 8.77053 6.62672 8.88528 7.31519L9.4819 10.8949C9.52406 11.1479 9.74294 11.3333 9.99941 11.3333C10.2559 11.3333 10.4748 11.1479 10.5169 10.8949Z" /></SvgIcon>;
}

function ChevronIcon() {
  return <svg className={styles.chevron} width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m3.5 4.5 2.5 2.5 2.5-2.5" /></svg>;
}

function CheckIcon() {
  return <SvgIcon><path d="m3 8.2 3 3 7-7" /></SvgIcon>;
}
