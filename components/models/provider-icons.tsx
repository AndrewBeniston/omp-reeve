import styles from "./provider-icons.module.css";
...
export function ProviderIcon({ id, size }: { id: string; size: number }) {
  const brand = PROVIDER_ICONS[id];
  if (!brand) {
    return (
      <span
        aria-hidden="true"
        className={styles.providerFallbackIcon}
        data-size={size}
      >
        {providerInitials(id)}
      </span>
  ...
  if (brand.hasColor) return <brand.Icon size={size} />;
  return <brand.Icon size={size} className={styles.monoProviderIcon} />;
}
