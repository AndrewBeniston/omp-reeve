...
const { buildActivePath, compressChain, selectTopLevelBranches } = await jiti.import("./BranchNavigator.tsx");
...
test("active paths include projected compressed entry identifiers", () => {
  const root = {
    entry: msg("a1", "assistant", "回答"),
    children: [],
    compressedEntryIds: ["u1", "tool1"],
  };
  assert.deepEqual([...buildActivePath([root], "tool1")], ["a1"]);
});

test("branch controls expose pressed and expanded state through ARIA", () => {
  assert.match(source, /<HeaderAction[\s\S]*?className=\{triggerClassName\}/);
  assert.match(source, /aria-pressed=\{open\}/);
  assert.match(source, /aria-expanded=\{open\}/);
  assert.match(navigationCss, /\.branchActionContent/);
});

test("branch rows expose native keyboard controls and active state", () => {
  assert.match(source, /<button[\s\S]*?className=\{styles\.branchTreeRow\}/);
  assert.match(source, /aria-current=\{isActive \? "true" : undefined\}/);
  assert.match(source, /onClick=\{\(\) => onSelect\(representative\.entry\.id\)\}/);
});

test("embedded branch content omits its duplicate disclosure header", () => {
  assert.match(source, /if \(embedded\) return panelContent;/);
});
