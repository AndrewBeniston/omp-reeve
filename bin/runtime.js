...
function getUnsupportedNodeVersionMessage(version) {
  return [
    `Reeve requires Node.js ${MIN_NODE_VERSION} or newer.`,
...
function getUnsupportedBunVersionMessage(version) {
  return [
    `Reeve requires Bun ${MIN_BUN_VERSION} or newer.`,
...
function getMissingBunMessage() {
  return [
    "Reeve could not find a Bun runtime.",
    "",
    "The omp SDK (@oh-my-pi/pi-*) is distributed as TypeScript sources and uses",
    "Bun-only builtins, so Reeve serves its API routes on Bun, exactly like",
    "the omp CLI itself.",
    "",
    ...
    "",
    "Already installed somewhere unusual? Point Reeve at it:",
    "  OMP_WEB_BUN=/path/to/bun reeve",
  ].join("\n");
}
