...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { buildServerEnvironment } = require("./server-environment");
const {
  getWebAuthStatus,
  resolveWebAuthFile,
  setWebPassword,
...
function requireTerminal(flag) {
  ...
  throw new Error([
    `${flag} has to ask for a password, but Reeve is not attached to a terminal.`,
    "Run reeve once interactively to set the password, or pass OMP_WEB_PASSWORD in the environment.",
  ].join("\n"));
}

async function setPasswordInteractively(intro) {
  console.log(intro);
  console.log(`It is stored as a scrypt hash in ${webAuthFile}. Reeve never keeps the password itself.`);
  const password = await readNewPassword();
  const status = setWebPassword(password, { file: webAuthFile });
  console.log(`Password saved. Reeve now asks for the username "${status.username}" and this password.`);
}
...
async function configurePasswordAccess() {
  ...
  if (resetPassword) {
    ...
    await setPasswordInteractively("Setting a new Reeve password.");
    return;
  }
  ...
  await setPasswordInteractively("--authenticated was requested and no Reeve password has been set yet.");
}
...
function warnAboutExposure() {
  ...
  if (getWebAuthStatus({ file: webAuthFile }).enabled) {
    console.warn(
      `Warning: Reeve is listening on ${hostname} with Basic Auth over HTTP. Use HTTPS or a trusted VPN to protect the password in transit.`,
    );
  } else {
    console.warn(
      `Warning: Reeve is listening on ${hostname} without authentication. Only use this on a trusted network, or start it with --authenticated.`,
    );
  }
}
...
function startServer() {
  ...
  const child = spawn(bunPath, ["--bun", nextBin, "start", "-p", port, "-H", hostname], {
    ...
    env: buildServerEnvironment(process.env, {
      hostname,
      webAuthFile,
  ...
  child.on("error", (error) => {
    console.error(`Failed to launch Reeve through Bun (${bunPath}): ${error.message}`);
    process.exit(1);
  });
