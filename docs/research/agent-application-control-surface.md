...
Eighty capabilities have no owning Reeve ticket.

Two of them need no ticket, because Reeve already delivers them through the agent runtime.

Those two marks are an inference.

Each entry below gives the capability and its scope in one sentence.
...
### List A. Capabilities that need a new ticket

This list proposes 20 new tickets.

Each entry names the capability numbers from the list above.
...
11. Agent settings read and write. Name the scope, the key, and the accepted values, and refuse a managed key. Covers 24, 25, 27, 28, 29, 30, and 31. A new epic is needed.
12. Human confirmation for an agent settings write. Refuse a second pending request, and refuse a request after the turn ends. Covers 26. A new epic is needed.
13. Sandbox policy control for an agent file write. Apply the three policy values to every agent write. Covers 10. A new epic is needed.
...
16. Agent Session creation and listing. Create a Session, list Sessions, list archived Sessions, and list projects. Covers 38, 39, 40, and 49. A new epic is needed.
17. Agent Session reading, waiting, and messaging. Read turns, wait over several Sessions, and send a follow-up prompt. Covers 41, 42, and 43. A new epic is needed.
18. Agent Session archive, rename, and pin. Run each change in the background. Covers 46, 47, and 48. A new epic is needed.
19. Agent Session move and window navigation. Move a Session, report the move status, and navigate the window. Covers 44, 45, and 50. A new epic is needed.
20. Visualization root, detection, and isolated view. Grant the per turn root, detect the write, render the view, and offer the repair action. Covers 54, 55, 56, 57, 58, and 59. A new epic is needed.

### List B. Capabilities whose existing ticket needs a rewrite

This list proposes 8 ticket rewrites.

1. Ticket 144. The reference registers the sub-agent panel as a right side panel Tab. The ticket opens the Tab in the bottom placement. This conflict is an inference.
...
6. Ticket 248. The classifier table must add the application control tools. Without those rows, agent panel calls render as generic cards.
...
### List C. Capabilities that need a maintainer decision from Andrew

This list raises 16 maintainer decisions.
...
No worker ran Codex Desktop, OMP, or Reeve in this session.

Every finding in this report rests on source reading.

The surface workers read the installed Codex Desktop archive.

The mapping worker read the installed OMP package source, version 18.1.6.

The ownership worker read the open Reeve issues with the command line tool.

The named unverified items are these.

- No worker performed a live application test on any surface.
- No worker verified any capture result on screen.
- No worker verified the Windows capture path on a Windows computer.
- No worker traced the internal behaviour of every window host service.
- The purpose column of the host service table is an inference.
- No worker confirmed which sibling report claims each shared service.
- No worker verified the rendered panel, the Tab strip, or the maximise state on screen.
- No worker verified how the file Tab identifier is derived from the file path.
- No worker verified the storage medium behind the client side key value store.
- No worker verified whether another build returns the fuller layout record to the agent.
- No worker verified cross window Tab movement, although two Tab types declare a window transfer.
- No worker verified the feature gate that controls the bottom host for a drag.
- No worker verified that a click on a file citation opens a file Tab.
- The core patch registration point for a file change is an inference.
- No worker verified the live appearance of the created task element.
- No worker verified the visible handoff progress item in the running application.
- No worker verified the pinned tool behaviour with the sidebar sections switch active.
- No worker verified the goal view scope contents.
- No worker verified the core goal tool implementation, because the archive does not contain it.
- The mapping worker did not inspect the Codex Desktop archive, and used the surface reports instead.
- The mapping worker read the OMP language service tool registration only.
- The mapping worker did not verify the exact count of OMP settings keys.
- The mapping worker did not verify OMP behaviour under a host other than RPC mode, except one elicitation path.
- The sweep worker did not trace the internal behaviour of every host service.
- Every judgement about partial Reeve ticket coverage is an inference.
- The report version 26.908.40834 of the reference extract is not independently verified.
