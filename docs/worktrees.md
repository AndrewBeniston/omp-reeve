...
Reeve can show all Git worktrees for one project in the sidebar. Use this when you want to keep separate checkouts for different branches, while keeping the project's sessions grouped together.
...
Use the worktree switcher to choose which checkout Reeve should use for new work in that project.
...
```text
/Users/alex/Documents/Workspace/omp-reeve
```

and you create branch `codex/worktree-help`, the worktree is created under:

```text
/Users/alex/Documents/Workspace/omp-reeve-worktrees/codex-worktree-help
```

If the branch already exists, Reeve adds a worktree for that branch. If it does not exist, Reeve creates the branch from the current `HEAD`.
...
- The Git branch.
- Reeve session history.
- The main checkout.

If the worktree has uncommitted or untracked files, Git refuses the removal. Reeve then offers a force remove action. Force removal discards the uncommitted files in that checkout, so use it only when you no longer need those changes.

## Sessions and Worktrees

Reeve groups sessions by project root, so sessions from the main checkout and linked worktrees appear together.
...
Git can keep prunable worktree records after a checkout disappears. Reeve filters those out of the switcher.
