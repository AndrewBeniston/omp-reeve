# Project trust in Reeve

## Why Reeve has a trust gate and `omp` does not
...
Reeve reaches the same code from a browser tab. Clicking a project in the
...
without anything that reads as consent. So Reeve keeps a trust store that omp
...
(`lib/session-system-prompt.ts`): Reeve resolves them for a browser session
...
When a project needs trust and has not been trusted, Reeve still runs omp's
...
Trust is granted from the dialog Reeve shows when you open such a project, and
...
The store is Reeve's own. Trusting a project in the browser does not change
