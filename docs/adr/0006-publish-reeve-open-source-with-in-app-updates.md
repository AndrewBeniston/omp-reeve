...
8. Updates. electron-updater. Check on launch and every four hours. Download in the background. Show the sidebar card from DESIGN.md 15.1 with `Restart now`. One stable channel. **The provider named here was the GitHub Releases provider. ADR-0013 replaces it with one feed per platform. Everything else in this point still holds.**
...
13. Where packages are built (2026-09-09, corrected 2026-09-10 and 2026-09-11). Packages are built on machines Andrew owns, named by role: a Mac for both macOS targets, a Windows machine for the Windows installer, and a Linux build machine for the Linux AppImage. The Windows build hang is fixed and issue #8 is closed, so Windows no longer needs the Linux machine and Wine. `gh release` uploads the packages. The GitHub workflow starts only by hand from the Actions tab and never on a tag push. Reason: GitHub bills macOS runners at 10x and Windows at 2x clock time. Six failed runs on 2026-09-09 cost about 2000 minutes for a build that takes 13 minutes and nothing on the Mac. An agent never starts the workflow without explicit approval for that run.
14. The public repository (2026-09-10). The plan in point 2 was not followed at first: 0.5.0 was pushed with the full private history. The same day the private repository was renamed `omp-reeve-archive` and a fresh `omp-reeve` was created from the tree of the release commit, with one commit, one tag, and the Release re-created from the local packages. The archive stays private and can be deleted. Reason: Andrew asked that no earlier commit be visible.
15. Build path leak (2026-09-10). Every 0.5.0 package carries the absolute path of the machine that built it, because Next writes the project directory into the server bundle. Fix in issue #6: build from a neutral path, and make the package verifier fail on `/Users/` or `/home/` in the built server. The pre-release scan checked the source tree and not the built output.
16. Branch protection (2026-09-11). Point 2 is now true. `main` refuses a direct push, refuses a force push, and refuses deletion. A change lands through a pull request whose `Verify build` check passes. The rule binds an administrator as well, because every agent here holds an administrator token. Confirmed by a refused push on 2026-09-11. Strictness is off on purpose: `main` moves often, and a strict branch forces a merge before every merge. To repair a broken `main`, turn protection off in Settings, push, and turn it back on.
17. Where a Windows release is built (2026-09-12). The neutral path in point 15 was a folder at the root of the Windows system drive. It now sits beside the repository checkout, because that machine does many other things and a folder at the drive root belongs to none of them. The rule is unchanged: the path must name no person. The price is that the path shows the folder layout of the machine, because Windows has no shared temporary directory outside a home directory.
18. What a package carries (2026-09-12). The packaged file list named five files by hand and missed three modules that the desktop shell requires. The installed application stopped at a Windows dialog and never drew a window. Every `*.cjs` is packaged now, and a test walks the require graph from `main.cjs` and `preload.cjs`. Reason: a hand list fails silently, and the failure reaches an installed copy rather than the build.
19. Two release checks corrected (2026-09-12). The personal-path scan used a bare `/home/`, which matched the product route `/api/home` in four bundle files and refused a correct package. One rule lives in `scripts/build-path.mjs` now, and each pattern needs a name after the directory. A Package file name must also carry no space, because GitHub writes a space as a dot, and the Platform feed then names a file the Release does not hold.
