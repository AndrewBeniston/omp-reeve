...
export async function PATCH(
  ...
  const { id } = await params;
  try {
    const { name, archived, pinned } = await req.json() as { name?: string; archived?: boolean; pinned?: boolean };
    const hasName = typeof name === "string";
    const hasArchived = typeof archived === "boolean";
    const hasPinned = typeof pinned === "boolean";
    if (!hasName && !hasArchived && !hasPinned) {
      return NextResponse.json({ error: "name, archived, or pinned is required" }, { status: 400 });
    }
    ...
    if (hasPinned) {
      await setSessionPinned(readSessionHeader(filePath)?.id ?? id, pinned);
    }
    invalidateSessionListCache();
