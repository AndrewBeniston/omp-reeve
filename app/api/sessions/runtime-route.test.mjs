...
test("session routes filter archived Sessions and accept archive updates", () => {
  assert.match(listRoute, /searchParams\.get\("archived"\) === "1"/);
  assert.match(listRoute, /showArchived \? archived\.has\(s\.id\) : !archived\.has\(s\.id\)/);
  assert.match(detailRoute, /typeof archived === "boolean"/);
  assert.match(detailRoute, /setSessionArchived\(readSessionHeader\(filePath\)\?\.id \?\? id, archived\)/);
});

test("session routes expose and update pinned Sessions", () => {
  assert.match(listRoute, /await loadPinnedSessionIds\(\)/);
  assert.match(listRoute, /pinned:\s*pinned\.has\(session\.id\)/);
  assert.match(listRoute, /Number\(Boolean\(right\.pinned\)\) - Number\(Boolean\(left\.pinned\)\)/);
  assert.match(detailRoute, /typeof pinned === "boolean"/);
  assert.match(detailRoute, /setSessionPinned\(readSessionHeader\(filePath\)\?\.id \?\? id, pinned\)/);
});
