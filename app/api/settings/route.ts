...
import { WEB_SETTINGS_FIELDS } from "@/lib/settings-api";
import { isApprovalMode } from "@/lib/approval-mode";
import { applyApprovalModeToRpcSessions } from "@/lib/rpc-manager";
...
export async function GET(req: Request) {
  try {
    ...
    fields.push(...WEB_SETTINGS_FIELDS.map((field) => ({ ...field })));
...
export async function PATCH(req: Request) {
  try {
    ...
    const browserField = WEB_SETTINGS_FIELDS.find((field) => field.owner === "browser" && field.path === body.path);
    if (browserField) {
    ...
    if (path === "tools.approvalMode" && isApprovalMode(value)) {
      settings.override("tools.approvalMode", value);
      applyApprovalModeToRpcSessions(value);
    }
