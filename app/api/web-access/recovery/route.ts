 ...
 * server's own console. The terminal running Reeve receives the code.
 * Completing the flow proves the caller can see that machine. A port scanner that finds this route
...
export async function POST(req: Request) {
  ...
  if (status.managedByEnvironment) {
    return NextResponse.json({
      error: "Password access comes from the OMP_WEB_PASSWORD environment variable, which Reeve cannot reset."
  ...
  if (status.unreadable) {
    return NextResponse.json({
      error: `The credential file at ${status.file} could not be read.`
        + " Run `reeve --reset-password` on the server to replace it.",
    }, { status: 409 });
  }
  if (!status.stored) {
    return NextResponse.json(
